...
impl SN76489 {
    ...
    pub fn update(&mut self, buffer: &mut Vec<Vec<i32>>, length: usize) {
        for j in 0..length {
            // Tone channels
            for i in 0..3 {
                if (self.mute >> i) & 1 != 0 {
            ...
            for i in 0..4 {
                // left
                buffer[0][j] = self.limit_u32(buffer[0][j] as f32 + self.channels[i]);
                // right
                buffer[1][j] = self.limit_u32(buffer[1][j] as f32 + self.channels[i]);
            }
